...
export default function UshFrqPage() {
  ...
  async function generatePrompt() {
    ...
    try {
      const res = await fetch("/api/frq-generate", {
        ...
        body: JSON.stringify({ subject: "ush" }),
      });
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 980, margin: "0 auto" }}>
        <Link href="/ush" style={{ color: "#fff", fontWeight: 800, textDecoration: "none" }}>
          ← Back
        </Link>

        <h1 style={{ fontSize: 42, fontWeight: 900 }}>
          APUSH Writing Studio
        </h1>

        <p style={{ color: "#ccc", marginBottom: 20 }}>
          Generate an APUSH SAQ, LEQ, or DBQ-style prompt, write your response,
          and get rubric-based grading.
        </p>
        ...
        >
          ...
          >
            {prompt || "Click Generate Prompt to load an APUSH writing prompt."}
          </div>
        </div>
        ...
        <button
          ...
          disabled={!prompt.trim() || !response.trim() || loadingGrade}
          style={{
            padding: "10px 16px",
            borderRadius: 10,
            border: "1px solid #fff",
            ...
            fontWeight: 800,
            cursor: !prompt.trim() || !response.trim() || loadingGrade ? "not-allowed" : "pointer",
          }}
        >
